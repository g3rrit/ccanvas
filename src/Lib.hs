...
module Lib (Color(Color), execCS, putPixel, clearScreen, loop)  where
...
foreign import ccall "clear_screen" c_clearScreen :: Ptr () -> CUChar -> CUChar -> CUChar -> CUChar -> IO ()
...
clearScreen :: Color -> CS ()
clearScreen (Color r g b a) = do
  cc <- get
  liftIO $ c_clearScreen cc (charCUChar r) (charCUChar g) (charCUChar g) (charCUChar a)
